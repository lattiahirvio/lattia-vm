...
// ask for the stack and stack pointer in case the functions must be used for
// multiple stacks
...
void swap(VM *vm, int to_SWAP) {
  ...
  if (vm->sp - to_SWAP < 0) {
    printf(
        "\e[0;31mError: SWAP value exceeds the size of the stack.\e[0;37m\n");
    printf("to_SWAP is %d\n", to_SWAP);
    return;
  }
    ...
    case SWAP: {
      ...
      if (debug)
        printf("b1 is %d, SP is %d, sp-b1 is %d, at SP-1 %d\n", b1, vm->sp,
               vm->stack[(vm->sp - 1) - b1], vm->stack[vm->sp - 1]);
      if (vm->sp < 2) {
      ...
      if ((vm->sp - 1) - b1 < 0 || b1 > vm->sp || b1 >= vm->sp) {
        printf("\e[0;31mError: SWAP value exceeds the size of the "
               "stack.\e[0;37m\n");
        printf("b1 is %d\n", b1);
        break;
      }
